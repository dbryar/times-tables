...
const prefix = (key: string, usePrefix: boolean): string =>
  usePrefix ? `${pkg.name ?? "unknown"}.app.${key.replace(r, "")}` : key.replace(r, "");
...
const get = <T = unknown>(key: string, initialValue?: StorageItem, usePrefix = true): T => {
  logger.debug("localStore: get() request", { key, usePrefix, initialValue });
  if (localStore.has(key, usePrefix)) {
    const [sType, sVal] = localStorage.getItem(prefix(key, usePrefix))?.split(/\|(.*)/s) ?? [
      "undefined",
      "",
    ];
...
const clearLocal = (): void => {
  logger.debug("localStore: clearLocal() searching", localStorage.length, "item(s)");
